...
Enter your new account's access key, secret key, and region (`us-east-1` is the default region in the `variables.tf` file --- change it in `variables.tf` if you're using a different region).

### Step-by-step

**1\. Enter the infra folder**
...
**7\. Cleanup when done to avoid charges**
